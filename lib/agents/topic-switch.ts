...
export function isServiceTopicSwitch(body: string) {
  ...
  if (
    ...
    /(?:קיבלתי|הגיע(?:ה|ו)?).*(?:פגם|פגום|קרוע|שבור|ליקוי|כתם|ריח|לא\s+תקין|לא\s+בסדר|מוזר|בעיה)/i.test(
      text
    ) ||
    /(?:קיבלתי|הגיע(?:ה|ו)?).*(?:רוצ(?:ה|ים|ות)\s+(?:ל)?(?:ה)?(?:חזיר|החזר)|(?:ל)?החזיר)/i.test(
      text
    ) ||
    /(?:קיבלתי|הגיע(?:ה|ו)?).{0,80}(?:אבל|ויש|וזה|עם).{0,40}(?:בעיה|לא|מוזר|כתם|חסר|לא\s+נכון)/i.test(
      text
    ) ||
    /(?:קיבלתי|הגיע(?:ה|ו)?)\s+רק|רק\s+חלק\s+מ(?:ן|)?(?:ה)?הזמנה|חסר(?:ים|ה)?\s+(?:לי\s+)?(?:פריט|מוצר|שטיח)/i.test(
      text
    ) ||
...
export function isSalesTopicSwitch(body: string) {
  ...
  if (
    /רוצ(?:ה|ים|ות)\s+לקנות|מחפש(?:ים|ת|ים)?(?:\s+ל(?:קנות|רכוש))?\s*(?:שטיח|פוף|תמונ|אביזר)?/i.test(text) ||
