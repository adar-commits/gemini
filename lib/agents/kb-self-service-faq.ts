...
export function customerExplicitlyRequestsHuman(body: string) {
  ...
  return /(?:^|\s)(?:נציג|יועץ|בן\s+אדם|אנושי|human)(?:\s|$|[?.!,])|(?:ל)?(?:דבר|דברו)\s+ע(?:ם|ם)\s+(?:נציג|יועץ)|(?:ה)?עביר(?:ו|י)?\s+(?:ל)?(?:נציג|יועץ)|(?:רוצ(?:ה|ים|ות)|(?:מ)?(?:עונ(?:ה|ים|ת)?|בקש(?:ה|ת)?))\s*(?:ל)?(?:נציג|יועץ)|מ(?:ענה|עונ(?:ה|ים|ת)?)\s+(?:מ)?(?:נציג|אנושי)|(?:לא|ולא)\s*(?:מ)?בוט|(?:אני\s+)?(?:רוצ(?:ה|ים|ות)\s+)?(?:ל)?דבר\s+ע(?:ם|ם)\s+נציג/i.test(
    text
  )
}
