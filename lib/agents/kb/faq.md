# HoM GROUP official knowledge base

Use only facts written here. If a fact is not here, treat it as unknown.
Do not merge separate sections into a new rule.
Company: קבוצת הום קמעונאות בע"מ, ח.פ. 515713212.
Brands: השטיח האדום (https://www.carpetshop.co.il), Pozitive / Positive (https://www.pozitiveshop.co.il), ELITE.
Both brands can be purchased in one cart.

## Contact
Customer service hours: א'-ה' 09:00-16:00.
Phone: *3076
Email: service@hom-group.co.il
Address-change WhatsApp listed in shipping policy: 077-9725055
Design chat hours: א'-ה' 10:00-17:00
Accessibility coordinator: רועי דוד, roy@carpetshop.co.il, כנרת 10, קריית שדה התעופה לוד.
Contact form: https://www.carpetshop.co.il/pages/contact-us

## Store hours
Network stores: א'-ה' 09:30-19:30. Airport City only until 18:00.
Friday: 09:00-14:00.
Source: https://www.carpetshop.co.il/pages/%D7%A1%D7%A0%D7%99%D7%A4%D7%99%D7%9D-%D7%94%D7%A9%D7%98%D7%99%D7%97-%D7%94%D7%90%D7%93%D7%95%D7%9D

## Branches — השטיח האדום / shared network
- באר שבע — המשק 1, מתחם ביג באר שבע. 053-3787354. א'-ה' 09:30-19:30, ו' 09:00-14:00.
- ראשון לציון — לישנסקי 10. 054-7109910. א'-ה' 09:30-19:30, ו' 09:00-14:00.
- נתניה — מפי 1. 054-7109808. א'-ה' 09:30-19:30, ו' 09:00-14:00.
- בני ברק — לח"י 9. 054-7109909. א'-ה' 09:30-19:30, ו' 09:00-14:00.
- פתח תקווה (צומת סגולה) — הרב פינטו מסעוד אשר 22. 053-5777861. א'-ה' 09:30-19:30, ו' 09:00-14:00.
- איירפורט ומטה החברה — כנרת 10, איירפורט סיטי. 053-5239921. א'-ה' 09:30-18:00, ו' 09:00-14:00.
- קריית אתא — דרך חיפה 44, מתחם Redesign. 054-9154541. א'-ה' 09:30-19:30, ו' 09:00-14:00.
Pozitive branch list: https://www.pozitiveshop.co.il/pages/%D7%A1%D7%A0%D7%99%D7%A4%D7%99%D7%9D-%D7%94%D7%A9%D7%98%D7%99%D7%97-%D7%94%D7%90%D7%93%D7%95%D7%9D
Pozitive listed stores: ראשון לציון, נתניה, בני ברק, פתח תקווה, איירפורט סיטי, קריית אתא.

## Payments — from FAQ page only
Source: https://www.carpetshop.co.il/pages/faq
- Up to 12 interest-free payments on every purchase.
- Website and stores: credit cards (Visa, Mastercard, American Express, IsraCard, tourist cards), digital wallets (Apple Pay, Google Pay), BUYME vouchers, Mifal HaPais (השטיח האדום products only), Hever card.
- Website only: bit.
- Stores only: bank transfer.
- Checks are not accepted at all.
BUYME: multiple BUYME vouchers can be used in one transaction. Cannot combine BUYME with another benefit type (Hever or Mifal HaPais).
Mifal HaPais: השטיח האדום products only. One voucher per transaction. Cannot combine with another benefit.
Hever: one benefit per transaction. Cannot combine with another benefit.

## Shipping policy
Source: https://www.carpetshop.co.il/policies/shipping-policy
Updated 29.07.26
- Free home delivery for rugs on purchases over 199 ILS. Under 199 ILS: 29.90 ILS.
- Free home delivery for poufs בהרכבה עצמית on purchases over 199 ILS. Under 199 ILS: 29.90 ILS.
- Ready poufs: 100 ILS home delivery.
- Free self-pickup for certain products from company warehouse, as stated below.
- Delivery up to 3rd floor without elevator at no extra charge (pilotis counts as a floor). Extra charge from the 4th floor.
- If the truck cannot reach the address, delivery may be by pickup truck at extra cost or at an agreed meeting point.
- Extra costs are paid to the courier at delivery.
- Customer may choose home delivery or self-pickup during checkout.
- Green tax invoice is sent by SMS when products are packed for shipment.
- Company supplies only a product paid in full by credit card.
- Delivery times for rugs and poufs בהרכבה עצמית: up to 4 business days.
- Delivery times for ready poufs: up to 14 business days.
- Times counted from credit-card / payment approval, the later of the two, business days only, or another date stated on the order.
- Friday, Saturday, holiday eves, holidays and Israeli memorial days are not business days.
- Orders placed after 12:00 move to the next business day.
- Times do not apply to out-of-stock products.
- Possible delays for oversized rugs 240/340 and 300/400.
- Service applies to delivery areas between Kiryat Gat and Zichron Yaakov.
- Additional delay possible for remote towns and towns beyond the Green Line, Jordan Valley, Arava and Eilat.
- Courier coordinates the delivery time by phone.
- A specific delivery hour cannot be pre-booked in advance; courier coordination is done on delivery day near arrival.
- Changing the address after handover to the courier costs money. Contact WhatsApp 077-9725055 or *3076.
- If the customer asks the courier to leave the parcel at the door, the company is not responsible for damage or loss.
- Damage or defect must be reported within 3 days of receiving the product.
- Special-order / pre-order products have longer production-based delivery dates shown on the product page.
- Products in a store and not in the warehouse may take up to 14 business days.
Self-pickup:
- Free for website customers only.
- Available only for certain products (rugs that are not OOAK, and poufs בהרכבה עצמית).
- Pickup only after SMS/email/WhatsApp ready notice.
- Customer must show the pickup notice and invoice.
- Rugs must be collected within 5 business days of ready notice. If not, the company may cancel without charging for the products. Repeat more than 3 times: company may block future website purchases.
- Warehouse pickup hours (owner-confirmed): 08:00-15:45.

## Owner-verified operational clarifications (from Goku Q&A)
- These answers were provided directly by the business owner and should be preferred when phrasing customer replies in overlapping cases.

### Delivery status wording for customers
- Customer-facing delivery comes **only** from `ZPIT_DELSTATUSCODE` (codes 1, 3, 4, 5, 6, 21, 22, 23, 80).
- Never tell a customer the order was delivered from `ORDSTATUSDES` ("הושלם" / "נמסרה") or from a date field.
- Unmapped codes (including 15 הוקפא זמנית) or a missing code: the order was found, but a clear delivery status cannot be shown, and the request is forwarded to the team.

### Damaged product initial flow
- For a "product arrived damaged" report: first run order lookup, then request a product photo.
- After order identification plus photo collection, offer transfer to customer service advisor for handling.

### Trade-in questions
- **No trade-in program** — there is no active trade-in / טרייד אין service or campaign at HoM.
- **Answer shape (one short sentence):** say we don't offer trade-in. In a **sales product inquiry**, continue sales intake on the next line — do not stop at policy.
- **Never invent alternatives** — do **not** mention rug repair (תיקון שטיחים / מתקנים שטיחים). HoM does not offer repair as a trade-in substitute unless this KB is updated by the operator.
- **Never unprompted pivot** to return/exchange/cancellation policy (14 days, החלפה, פורטל) when they only asked about trade-in — that is a different topic.
- Optional: `get_campaigns` — if nothing matches trade-in, say there is no such campaign (never invent one).
- If they want a human to discuss purchase options → sales advisor (`human_sales` after intake or on explicit request).

## Refund / exchange / cancellation policy
Source: https://www.carpetshop.co.il/policies/refund-policy
Updated 07.07.25

### Bot reply wording (returns / cancellations) — operator locked
When explaining how to cancel or return:
- **Always name both execution paths** (not portal-only): (1) **החזרה בסניפי הרשת** — ללא עלות; (2) **איסוף שליח מהבית** — בתשלום לפי גודל השטיח (85–300 ₪ לכיוון — see fee table below). **Both paths** require opening the request in the returns portal first (also when returning at a branch).
- **Portal is mandatory** — the customer must open the request themselves in the returns portal. Say **"יש לפתוח בקשת ביטול/החזרה בפורטל"** with the link; in the portal they choose branch drop-off or home pickup. **Never** "אפשר לפתוח בקשה" — sounds optional.
- **Product condition wording** — say **"ללא שימוש, באריזתו המקורית"** within the 14-day window. **Do not** use **"מוצר שלם"**, **"אם המוצר שלם"**, or **"שלם"**.
- Refund timeline: up to **7 business days from cancellation** (ממועד ביטול העסקה).
- **"ביטול עסקה" example shape:** policy (14 days + conditions) → **two ways to return** (branch / courier via portal) → portal link with phone prefill → refund timeline (7 business days). Optional: ask which order if unclear. Do **not** dump full branch address list unless they ask.
- Exchange/return at any network store with receipt, tax-receipt invoice, or order proof.
- Exchange/return within 14 days from receiving the product or from receiving written transaction details, the later of the two.
- Company lab must confirm the product was unused and has no defect or dirt. Returned products go to the company lab.
- Exchange/return Sunday-Thursday only, subject to store hours.
- No exchange/return on Friday, Saturday, holiday eves, shutdown days, holidays and Jewish festivals.
- If the customer returns by shipment, the customer is charged one delivery direction only (the return). No extra shipping is charged for the new product.
- Pozitive products: exchange/return at network stores only (no shipment).
- Store list: https://www.carpetshop.co.il/pages/%D7%A1%D7%A0%D7%99%D7%A4%D7%99%D7%9D-%D7%94%D7%A9%D7%98%D7%99%D7%97-%D7%94%D7%90%D7%93%D7%95%D7%9D
Cancellation:
- Not due to defect/mismatch: customer cancellation notice plus receipt/invoice within 14 days from receiving the product or order-confirmation notice, the later, and the product is intact, unused, undamaged.
- Cancellation notice must include customer name, ID number, transaction details and receipt invoice, by email service@hom-group.co.il, contact form https://www.carpetshop.co.il/pages/contact-us, or *3076.
- Company cancels the charge within 7 business days from cancellation.
- Return the packed product to a network store. If the customer cannot return it independently, the company charges shipping.
- Defect cancellation: notice plus invoice within 14 days from receipt or confirmation, the later. No shipping or cancellation fee. Alternatively credit for a future purchase or exchange for an identical or similar-value product if the customer waives refund.
- Credit can be used in stores / phone transactions only (not on the website).
- No cancellation for custom-made products, including anti-slip underlay cut to size.
- After opening, return in original packaging or sealed packaging of no lesser quality. No credit or exchange without packaging.
Cancellation costs:
- Customer-initiated cancel: refund of the price minus shipping if any.
- Cancel due to defect, mismatch, or late supply: full credit.
- Refund as soon as possible and no later than 7 business days from cancellation, subject to unused-product confirmation.
- Refund in the original payment method: cash → cash or bank transfer; card → charge cancelled / card company credited.
Storage: a stored-product deal cannot be cancelled if 14 days have passed since purchase.
Home pickup/delivery fees:
- Rug up to 160*230: 85 ILS per direction
- Rug 200*290: 100 ILS per direction
- Rug 240*340: 150 ILS per direction
- Rug 300*400: 300 ILS per direction
- Poufs: 100 ILS per direction
- Returns portal (returns/cancellations only — not for product exchanges): https://returns.carpetshop.co.il/

## FAQ page extra facts
Source: https://www.carpetshop.co.il/pages/faq
- Visualization: photograph the room or use a built-in photo, choose rug and size, then "הדמיה בחדר שלי". Visualization page: https://www.carpetshop.co.il/pages/visualization-page
- How to open packaging: carefully cut one plastic end with scissors, remove the rug, remove corner protectors, cut and remove tape. Do not use sharp objects on the rug.
- How to lay the rug: spread on the floor and flatten creases by hand or by stepping.
- Spot clean: alcohol-free wipe, or microfiber cloth in warm water and dish soap, circular motion. General clean: professional dry cleaning.
- Anti-slip underlay is recommended under every rug.
- Storage: roll, wrap in nylon, add moth protection, seal with air holes, store dry and shaded.
- Size may vary slightly because of finishing.
- Shedding/fluff on a new natural-fiber rug is normal and should lessen over months with regular vacuuming.
- Custom anti-slip underlay and made-to-order rugs cannot be cancelled.

## Carpet rental / temporary trial
- **Do not proactively offer** — mention only when the customer explicitly asks about השאלה / שכירה / להשאיל / לנסות בבית.
- זה אינו שירות קבוע שמוצע לכל לקוח.
- במקרים מסוימים, כשלקוח מתלבט בין שני דגמים, יועץ המכירות יכול לאשר שימוש זמני לפי שיקול דעתו (לעיתים לפי הדגם הזול מבין השניים) — מקרה-מקרה בלבד.
- לבדיקת זכאות להשאלת שטיח לתקופת ניסיון יש להפנות ליועץ מכירות.

## Online consulting terms
Source: https://www.carpetshop.co.il/pages/%D7%AA%D7%A7%D7%A0%D7%95%D7%9F-%D7%A9%D7%99%D7%A8%D7%95%D7%AA-%D7%99%D7%99%D7%A2%D7%95%D7%A5-%D7%90%D7%95%D7%A0%D7%9C%D7%99%D7%99%D7%9F-%D7%91%D7%90%D7%AA%D7%A8
Updated 16.03.26
- Online consulting is product-selection advice only.
- General recommendations only: product type, size, color, general fit to the home/space.
- Not professional interior design, architecture, engineering, supervision, plans, drawings, renderings, installation or execution.
- Given by phone, WhatsApp or another online channel chosen by the company.
- Up to 15 minutes.
- Subject to consultant availability and company discretion. No commitment to a specific time.
- Free and without purchase obligation.
- Customer is responsible for the accuracy of photos/information supplied.

## Accessibility
Source: https://www.carpetshop.co.il/pages/accessibility
Updated 05.02.26
- Site accessibility by "נגיש בקליק".
- Aims to meet Israeli Equal Rights for Persons with Disabilities (service accessibility) regulations 2013.
- Recommended screen reader: latest NVDA.
- Accessibility menu on the site.
- Works in Chrome, Firefox, Explorer 10+, Safari, Opera.
- Accessibility coordinator: רועי דוד, roy@carpetshop.co.il, כנרת 10.

## Privacy
Source: https://www.carpetshop.co.il/policies/privacy-policy
Updated 09.11.25
- Privacy contact: service@hom-group.co.il or *3076.
- Payment card details are not stored by the company; they are handled by the payment provider.
- Users may review/correct/delete personal data under the Privacy Protection Law 1981.

## Policy URLs to include when relevant
- Accessibility: https://www.carpetshop.co.il/pages/accessibility
- Terms: https://www.carpetshop.co.il/policies/terms-of-service
- Refunds: https://www.carpetshop.co.il/policies/refund-policy
- Shipping: https://www.carpetshop.co.il/policies/shipping-policy
- Privacy: https://www.carpetshop.co.il/policies/privacy-policy
- Online consulting: https://www.carpetshop.co.il/pages/%D7%AA%D7%A7%D7%A0%D7%95%D7%9F-%D7%A9%D7%99%D7%A8%D7%95%D7%AA-%D7%99%D7%99%D7%A2%D7%95%D7%A5-%D7%90%D7%95%D7%A0%D7%9C%D7%99%D7%99%D7%9F-%D7%91%D7%90%D7%AA%D7%A8
- FAQ: https://www.carpetshop.co.il/pages/faq
- Carpet terminology glossary: https://www.carpetshop.co.il/pages/%D7%9E%D7%99%D7%9C%D7%95%D7%9F-%D7%9E%D7%95%D7%A0%D7%97%D7%99%D7%9D
- Pozitive product FAQ: https://www.pozitiveshop.co.il/pages/faq
- Pozitive assembly/care videos: https://www.pozitiveshop.co.il/pages/pozitive-tutorial-videos
- About: https://www.carpetshop.co.il/pages/about
- Branches: https://www.carpetshop.co.il/pages/%D7%A1%D7%A0%D7%99%D7%A4%D7%99%D7%9D-%D7%94%D7%A9%D7%98%D7%99%D7%97-%D7%94%D7%90%D7%93%D7%95%D7%9D
- Visualization: https://www.carpetshop.co.il/pages/visualization-page
- Roomvo: https://www.roomvo.com/my/carpetshop/rooms/
- Returns portal: https://returns.carpetshop.co.il/

## Dated promotions — prefer live API via `getCampaigns`
- Call `get_campaigns` when customer asks about מבצעים, validity, or whether a campaign is still active.
- Static KB examples below are **fallback only** if the API is unavailable:
- השטיח האדום 50% terms: https://www.carpetshop.co.il/pages/%D7%AA%D7%A7%D7%A0%D7%95%D7%9F-%D7%94%D7%9B%D7%9C-%D7%91-50 — 50% on products at site and stores, not shipping. Valid 14.8.26 14:30 to 26.8.26 09:00 or while stock lasts.
- Pozitive 1+1 terms: https://www.pozitiveshop.co.il/pages/%D7%AA%D7%A7%D7%A0%D7%95%D7%9F-%D7%9E%D7%91%D7%A6%D7%A2-1-2 — buy two participating poufs/cushions, pay the higher price. Valid 11.8.26 09:30 to 18.8.26 09:00 or while stock lasts.
If today's date is after the stated end date, treat the promotion as ended.
