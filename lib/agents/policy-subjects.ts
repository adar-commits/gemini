...
export const POLICY_SUBJECTS: PolicySubject[] = [
  ...
  },
  {
    ...
    patterns: [
      /שכיר(?:ות|ת)|השכר(?:ה|ת)|לשכ(?:ור|יר)|rent(?:al)?|lease|להשאיל(?:\s+א(?:ת|ת))?\s+שטיח|שאיל(?:ת|ה)\s+שטיח|השאל(?:ת|ה)?\s+שטיח|השאל(?:ה|ות)(?:\s+של|\s+ל)?\s+שטיח|ל(?:נסות|try)\s+(?:לפני|before)\s+(?:קנ|buy)|לנסות\s+(?:את\s+)?(?:ה)?שטיח\s+(?:ב(?:בית|חדר)|ל(?:פני|תקופה))|נסיון\s+(?:לפני|בבית)|try[\s-]?before/i,
    ],
  },
]
...
ניתן לבצע החלפה בתוך 14 יום מקבלת המוצר — ללא שימוש, באריזתו המקורית.
...
בכל החזרה, גם כאשר מחזירים את המוצר בסניף, יש לפתוח בקשת החזרה דרך פורטל ההחזרות (בפורטל בוחרים סניף או איסוף שליח):
${portalUrl}
ניתן לבצע החזרה בתוך 14 ימים מקבלת המוצר — ללא שימוש, באריזתו המקורית. הזיכוי מתבצע עד 7 ימי עסקים ממועד ביטול העסקה.`
}
...
אם מדובר בפתיחת בקשת החזרה/ביטול חדשה, יש לפתוח את הבקשה בפורטל ההחזרות:
${portalUrl}
מה מתאים למצב שלכם?`
}
...
export function buildCarpetRentalPolicyReply() {
  return `בנוגע להשאלת שטיח לתקופת ניסיון — זה אינו שירות קבוע שמוצע לכל לקוח.
במקרים מסוימים, למשל כשמתלבטים בין שני עיצובים, יועץ המכירות יכול לבדוק אפשרות לאישור זמני (לעיתים לפי המוצר הזול מבין השניים) — מקרה-מקרה, לפי שיקול דעת היועץ בלבד.
