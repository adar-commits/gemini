...
const RETURN_INTENT_RE =
  /(?:רוצ(?:ה|ים|ות)|(?:מ)?(?:עונ(?:ה|ים|ת)?|בקש(?:ה|ת)?))\s*(?:ל)?(?:ה)?(?:חזיר|החזר|בטל|ביטול)|(?:נוכל|אפשר|מותר)\s+(?:ל)?(?:ה)?(?:חזיר|החזר|בטל|ביטול)|(?:ל)?(?:ה)?(?:חזיר|בטל|ביטול)(?:\s+(?:ב(?:[א-ת]+|\d)|א(?:ת|ת)?|אות(?:ו|ה|ם)?))|(?:ב(?:ק|ק)ש(?:ה|ת)?\s+)?(?:ה)?החזר(?:ה|ות)?(?:\s|$)|(?:^|\s)(?:ביטול|זיכוי)(?:\s|$|[?.!,])|(?:^|\s)(?:ל)?(?:בטל|ביטול)(?:\s|$|[?.!,])/i
...
const DISSATISFACTION_RE =
  /לא\s+(?:ממש\s+|כל\s+כ(?:\"|״|')?ך\s+|מ(?:די)?\s+)?(?:מרוצ|(?:מ)?(?:ת)?(?:י)?(?:ת)?אים|א(?:וה|ה)ב(?:ת|ים|ות|ו)?|כ(?:\"|״|')?כ)|לא\s+א(?:וה|ה)ב\s+א(?:ת(?:ו|ה|ם)?|ה(?:שטיח|מוצר)|אות(?:ו|ה))|אי[\s-]?שביעות\s+רצון|לא\s+מה\s+ש(?:ציפיתי|ציפינו|ציפית)|(?:ל)?צער(?:י|נו)\s+(?:ש)?(?:אני|אנחנו)?\s*לא/i

const MISMATCH_RE =
  /צבע.*(?:לא\s+תואם|שונה|דהוי)|(?:לא\s+תואם|שונה\s+מ|דהוי).*?(?:אתר|תמונה|צבע)|נראה\s+שונה|שונה\s+בפועל|לא\s+כמו\s+ב(?:אתר|תמונה)|(?:פחות\s+)?(?:לא\s+)?(?:מ)?(?:ת)?(?:י)?(?:ת)?אים(?:\s+לי|\s+ל(?:נו|כם))?|גוונ(?:י|ים).*(?:שונ|ורוד|כה(?:ה|ים)|בהיר)|(?:רשום|באתר).{0,30}(?:הגיע|קיבל)/i
...
export function isReturnPolicyQuestion(text: string) {
  ...
  if (isReturnShippingFeeQuestion(trimmed)) return true

  if (
    /(?:תחרט|אתחרט)/i.test(trimmed) &&
    /(?:אקבל|יקבל|לקבל|הביתה|קיבל(?:תי|נו)?|הגיע)/i.test(trimmed)
  ) {
    return true
  }

  if (
    /(?:ע(?:דיין|וד)\s+לא\s+(?:הגיע|קיבל|מגיע)|טרם\s+(?:הגיע|קיבל))/i.test(trimmed) &&
    /(?:ל(?:בטל|ביטול)|ביטול|(?:ל)?(?:ה)?(?:חזיר|החזר))/i.test(trimmed)
  ) {
    return true
  }
...
/** Return courier / pickup fee ask — KB FAQ, not service handoff. */
export function isReturnShippingFeeQuestion(text: string) {
  const trimmed = text.trim()
  if (!trimmed) return false
  if (isActiveReturnExchangePickupCase(trimmed)) return false

  const feeAsk =
    /(?:כמה\s+(?:י(?:עלה|על))|(?:דמי|עלות)\s+(?:ה?(?:משלוח|הובלה|איסוף|שליח))|מחיר\s+(?:של\s+)?(?:ה?(?:הובלה|איסוף|משלוח)))/i.test(
      trimmed
    )
  const returnContext =
    /(?:החזיר|החזר|ביטול|תחרט|אתחרט|להחזיר|זיכוי|משלוח|איסוף|שליח|פורטל|לא\s+(?:מ)?(?:ת)?(?:י)?(?:ת)?אים)/i.test(
      trimmed
    )
  const futureReceiveRegret =
    /(?:קודם\s+)?(?:אקבל|יקבל|לקבל).{0,80}(?:ואז|ואחר(?:י)?|ואחר)\s*(?:אתחרט|תחרט)/i.test(
      trimmed
    )

  return (feeAsk && returnContext) || (feeAsk && futureReceiveRegret) || futureReceiveRegret
}
