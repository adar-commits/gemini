...
const DISSATISFACTION_RE =
  /לא\s+מרוצ|לא\s+מתאים|לא\s+א(?:וה|ה)ב(?:ת|ים|ות)?|לא\s+כ(?:\"|״|')?כ|אי[\s-]?שביעות\s+רצון|לא\s+מה\s+ש(?:ציפיתי|ציפינו|ציפית)|(?:ל)?צער(?:י|נו)\s+(?:ש)?(?:אני|אנחנו)?\s*לא/i
...
function matchesDissatisfaction(text: string) {
  ...
  if (
    RECEIVED_RE.test(text) &&
    /(?:לא\s+מרוצ|לא\s+מתאים|לא\s+א(?:וה|ה)ב(?:ת|ים|ות)?|לא\s+כ(?:\"|״|')?כ)/i.test(text)
  ) {
    return true
  }
  return false
