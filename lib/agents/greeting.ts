...
const BUSINESS_HINT_RE =
  /שעות|סני[פף]|מדיניות|משלוח|החזר|תשלום|איך\s+מחזיר|רוצה\s+לקנות|מחפש(?:ים|ת|ים)?\s+ל?(?:קנות|לקנות)|מחיר|שטיח|פוף|קרוע|פגום|לא\s+קיבלתי|משלוח(\s+שלי)?|הזמנה|פתוח|סגור|עד\s+מתי|מתי\s+פתוח|מחר|היום|כתובת|איפה|מיקום|קריית|איירפורט|קבלה|חשבונית|העתק|עותק|receipt|invoice|הנחה|מבצע|קופון|coupon|discount/i
...
export function sanitizeBotVoiceGender(text: string) {
  ...
  const replacements: Array<[RegExp, string]> = [
    ...
    [/(?<![\u0590-\u05FF])אני\s+מבינ(?:ה|ות)(?![\u0590-\u05FF])/g, "אני מבין"],
    [/(?<![\u0590-\u05FF])מבינ(?:ה|ות)(?=\s+א(?:ת|תם|תן|תכם|תכן)\s)/g, "מבין"],
    [/(?<![\u0590-\u05FF])מכוונ(?:ה|ת)(?![\u0590-\u05FF])/g, "מכוון"],
    [/(?<![\u0590-\u05FF])אוכלת(?![\u0590-\u05FF])/g, "אוכל"],
    [/(?<![\u0590-\u05FF])נשמח(?![\u0590-\u05FF])/g, "אשמח"],
