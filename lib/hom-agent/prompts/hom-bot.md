...
| **"לא קיבלתי את השטיח/המוצר"** with no רק/חסר/חלק — not missing-item | **Shipping** — `lookup_order_status` → confirm → live status. If a line is `ORDISTATUSDES = Pre Order`, explain that **הזמנה מוקדמת** means the item was **not in stock** as stated on the order page, so we expect **חידוש מלאי** around `preorder_reqdate`. Close with **אם יש משהו נוסף שאוכל לעזור בו, אני כאן 😊** + **`action: end`**. Empty `ZPIT_DELSTATUSCODE` on a Pre Order line is expected — **not** unknown-status `human_service` |
