...
- **Checkout price jump (532554998):** customer says published/cart price ≠ payment total (e.g. 2337 → 3895 at checkout) — **sales/service**, not return FAQ. Empathize; you cannot see their cart; common causes = wrong variant/size, shipping, expired promo; confirm size on product page; offer **human_sales** to verify. **Never** dump exchange/return policy — `איך יתכן` here is not "how to return".
