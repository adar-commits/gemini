...
| **"לא קיבלתי את השטיח/המוצר"** with no רק/חסר/חלק — not missing-item | **Shipping** — `lookup_order_status` → confirm → live status. If a line is `ORDISTATUSDES = Pre Order`, explain הזמנה מוקדמת + `preorder_reqdate` ETA. Empty `ZPIT_DELSTATUSCODE` on a Pre Order line is expected — **not** unknown-status `human_service` |
