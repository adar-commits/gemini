...
-- 2) Prompt cache read rate (Token Optimization Council — last 24h FAQ calls)
SELECT
  COUNT(*) AS faq_calls,
  SUM(cache_read_tokens) AS cache_read_total,
  SUM(cache_write_tokens) AS cache_write_total,
  ROUND(100.0 * SUM(cache_read_tokens) / NULLIF(SUM(input_tokens), 0), 1) AS cache_read_pct_of_input
FROM hom_agent_token_usage
WHERE created_at > now() - interval '24 hours'
  AND purpose = 'faq';

-- 3) Avg tokens per conversation per day
SELECT
...
-- 4) Master skip rate from shadow logs
SELECT
  routing_path,
  COUNT(*) AS turns,
...
-- 5) Route disagreement rate (master LLM vs guessMasterRoute)
SELECT
  agree,
  COUNT(*) AS rows,
...
-- 6) Top disagreements to review for T1 allowlist expansion
SELECT
  guessed_route,
  master_action,
  COUNT(*) AS n
...
-- 7) Never-stuck rate (adjust reply pattern if copy changes)
